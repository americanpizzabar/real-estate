...
export function cleanAddress(raw: string): string {
  ...
  // 注意: 「円」単体で切ると 渋谷区円山町・京都円町 等の地名を誤って切るため、
  // 金額は「数字＋(万)円」の形にアンカーする。
  s = s.split(
    /価格|販売価格|物件価格|売出価格|面積|土地面積|建物面積|延床|専有面積|間取|利回|表面利回|沿線|交通|最寄|築年|築年月|構造|総戸数|階建|[0-9０-９][0-9０-９,，.]*\s*万?円|㎡/
  )[0];
